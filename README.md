...
- Telegraf v4, [node-fetch](https://github.com/node-fetch/node-fetch) и [web-streams-polyfill](https://github.com/MattiasBuelens/web-streams-polyfill) — стек HTTP-клиента, через который загрузчик XLSX обращается к API Telegram. Для Node.js 16 автоматически подключается полифилл Web Streams, поэтому дополнительные флаги запуска не требуются.
...
5. Запустите проверки и поднимите бота (команды протестированы на Node.js 16.20.2 и 20.x):
   ...
   npm start        # запуск бота напрямую в Node.js (выполняет `node ./src/index.js`)
   ```
   ...
   В проекте нет стадии сборки: вся логика запускается из исходников JavaScript, поэтому `npm start`
   и `npm run dev` больше не требуют TypeScript-компиляции или запуска через `ts-node`.
...
При ошибках валидации рабочих JSON (например, повреждённый `deliveries.json`) полный отчёт с расшифровкой нарушений схемы сохраняется в `logs/validation-errors.log`. Пользователь в Telegram получает только сокращённую сводку, поэтому для диагностики открывайте файл журнала и ищите записи по времени инцидента.
